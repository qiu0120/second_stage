...
# Low-load/headless run mode:
# - suppress ROS terminal logs as early as possible
# - later monkey-patch OpenCV window calls to avoid GUI CPU/GPU overhead
import os
...
# 全局关闭 OpenCV 可视化窗口，避免 cv2.imshow / waitKey / namedWindow 带来的额外负载。
HEADLESS_MODE = True
if HEADLESS_MODE:
    def _cv2_no_window(*args, **kwargs):
        return None
    cv2.imshow = _cv2_no_window
    cv2.namedWindow = _cv2_no_window
    cv2.destroyWindow = _cv2_no_window
    cv2.destroyAllWindows = _cv2_no_window
...
class CombinedStage1Stage2Node(Node):
    def __init__(self):
        ...
        # self.declare_parameter('initial_state', 'BLIND_MARCH')
        # self.declare_parameter('initial_state', 'P5_FINAL_LONG_JUMP')
        # self.declare_parameter('initial_state', 'P5_RECOVERY_STAND')
        ...
        self.declare_parameter('p1_blind_left_vy', 0.17)
...
class FifthStageMixin:
    ...
    def p5_enter_state_after_blocking_wait(self, new_state: str):
        """
    ...
    # YAML 参数发布
    # ============================================================
    def p5_publish_yaml_vecxd(self, name: str, values: List[float], is_user: int = 1):
        msg = YamlParam()
        msg.name = name
    ...
    def set_body_roll_height(self, roll: float, height: float):
        ...
        self.p5_publish_yaml_vecxd('des_roll_pitch_height', values, is_user=1)
    ...
    def p5_send_action_once(self, mode: int, gait_id: int):
    ...
    def run_timed_velocity_then_stop_state(
        ...
        if hold_after_duration_until_aligned and use_inner_edge_align:
            ...
            if self.p5_forward_align_stable_counter >= self.p5_forward_after_reset_body_align_stable_frames:
                ...
                if stop_when_done:
                    self.p5_send_stop_command()
                    self.p5_enter_state_after_blocking_wait(next_state)
                else:
                    self.p5_enter_state(next_state)
                return
        ...
        try:
            if self.state == self.P5_RECOVERY_STAND:
                ...
                if self.p5_state_elapsed_s() >= self.p5_reset_body_wait_s:
                    ...
                    self.p5_enter_state(self.P5_RIGHT_SHIFT_BEFORE_RIGHT_JUMP)
...
class SixthStageMixin:
    def sixth_stage_init(self):
       ...
       # 👇👇👇 东进潜入调参区 (绕过球走到死角) 👇👇👇
        ...
        self.eastward_vy_time_s = 2.2       # 【横移持续时间】往北侧滑几秒后停止横移
...
# 🆕 新增：出口冲线与趴下参数
        ...
        self.p6_states = {
            ...
            'CROSS_FINISH_LINE', 'MISSION_COMPLETE',
        }
        self.p6_initial_state = 'BLIND_MARCH'
    ...
    def now_sec(self) -> float:
        """返回 ROS 当前时间；节点 use_sim_time=True 时就是 Gazebo 仿真时间。"""
        return self.get_clock().now().nanoseconds / 1e9
...
class FullCompetitionNode(FourthStageMixin, FifthStageMixin, SixthStageMixin, CombinedStage1Stage2Node):
    """
    ...
    def __init__(self):
        ...
        # 低负载运行：关闭可视化和普通终端日志。
        # 注意：RCUTILS_LOGGING_SEVERITY_THRESHOLD 已在文件顶部设置为 FATAL，
        # 这里再对当前节点 logger 设置一次，避免 info/warn 大量刷屏。
        self.low_load_mode = True
        self.show_debug_vis = False
        self.show_vision = False
        self.vision_window_ready = False
        try:
            if LoggingSeverity is not None:
                rclpy.logging.set_logger_level(self.get_logger().name, LoggingSeverity.FATAL)
        except Exception:
            pass
        ...
        # 有些 init 函数可能会重新声明/覆盖可视化开关，这里再统一关一次。
        self.show_debug_vis = False
        self.show_vision = False
        self.vision_window_ready = False

        self.enable_fifth_stage_after_fourth = True
        self.enable_sixth_stage_after_fifth = True
        self.get_logger().info('FullCompetitionNode ready: P1 -> P2/P3 -> P4 -> P5 -> P6 in one ROS2 node.')
    ...
    def is_fifth_stage_state(self, state: str) -> bool:
        return isinstance(state, str) and state.startswith('P5_')
    ...
    def enter_fifth_stage(self, reason: str):
        self.get_logger().info(f'[HANDOFF] P4 -> P5: {reason}')
        self.clear_pre_fifth_vision_caches()

        if self.show_debug_vis:
            try:
                cv2.destroyWindow('Fourth Stage Debug')
                cv2.destroyWindow('P3 yellow track debug')
                cv2.destroyWindow('part3_origin_debug')
                cv2.destroyWindow('part3_mask_mid')
                cv2.destroyWindow('part3_mask_near')
            except Exception as e:
                self.get_logger().warn(f'[HANDOFF] destroy P4/P3 debug windows failed: {e}')

        # Give P5 a clean timer/action state.
        self.action_sent = False
        self.state_timer_reset_after_blocking_wait = False
    ...
    def rgb_callback(self, msg: Image):
        ...
        if isinstance(self.state, str) and self.state.startswith('P1_'):
            ...
            if self.show_debug_vis:
                self.p3_show_debug_window(frame)
        elif self.is_sixth_stage_state(self.state):
            self.p6_image_callback(msg)
        elif self.is_fifth_stage_state(self.state):
    ...
    def control_loop(self):
        if self.is_sixth_stage_state(self.state):
            self.p6_control_loop()
            return

        if self.is_fifth_stage_state(self.state):
            self.p5_control_loop()
            if (
                self.enable_sixth_stage_after_fifth
                and self.state == self.P5_DONE
            ):
                self.enter_sixth_stage('fifth stage P5_DONE')
            return

        if self.is_fourth_stage_state(self.state):
            self.fourth_control_loop()
            # FourthStageMixin uses DONE as the end of P4. In the integrated node,
            # hand it off to P5 instead of keeping the robot stopped forever.
            if (
                self.enable_fifth_stage_after_fourth
                and self.state == self.DONE
            ):
                self.enter_fifth_stage('fourth stage DONE')
            return
