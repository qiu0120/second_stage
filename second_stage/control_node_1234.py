...
from fourth_stage.robot_control_cmd_lcmt import robot_control_cmd_lcmt as fourth_robot_control_cmd_lcmt
from fourth_stage.robot_control_response_lcmt import robot_control_response_lcmt as fourth_robot_control_response_lcmt
...
class CombinedStage1Stage2Node(Node):
    def __init__(self):
        ...
        # 程序刚开始保持 normal 机身高度；第四赛段入口再切换到 low。
        # 这里使用当前总节点创建 yaml_parameter publisher，不额外新建 ROS2 节点。
        self.fourth_yaml_node = FourthYamlPub(self)
        ...
        # 第四赛段控制器：不是新的 ROS2 节点，只在第三赛段结束后挂到当前节点上运行。
        self.fourth_stage = None

    def set_body_normal(self):
        """
        全流程启动时调用一次，把机身高度设置为 normal。
        第四赛段真正开始时，FourthStageController 会再调用 set_body_low()。
        """
        values = [0.0] * 12
        values[0] = 0.0   # roll
        values[2] = 0.25  # normal height

        self.fourth_yaml_node.publish_yaml_vecxd(
        ...
        # 保持和第四赛段 set_body_normal() 一致：发 normal 后站立稳定。
        self.send_stop_command()
        self.get_logger().info('[BODY] startup set normal height: 0.25')
    ...
    def p3_control_loop(self):
        ...
        if self.state == 'P3_ALIGN_TRACK':
            if elapsed >= self.p3_align_max_duration_sec:
                self.get_logger().info('P3_ALIGN_TRACK timeout, start fourth stage.')
                self.set_state('START_FOURTH_STAGE')
                return
            ...
            if self.p3_s4_valid > 0.5:
                ...
                if abs(err_lat) < self.p3_align_lat_tol and abs(err_yaw) < self.p3_align_yaw_tol:
                    self.get_logger().info('P3_ALIGN_TRACK complete: centered and aligned, start fourth stage.')
                    self.set_state('START_FOURTH_STAGE')
                    return
    ...
    # ============================================================
    # 第四赛段衔接：不创建第二个 ROS2 节点，只停止前三赛段 LCM 发送线程，
    # 然后在当前节点内部创建第四赛段控制器。
    # ============================================================
    def start_fourth_stage_in_same_node(self):
        if self.fourth_stage is not None:
            return

        self.get_logger().info('Stage 1+2+3 finished, starting fourth stage inside the same ROS2 node...')

        # 避免前三赛段的 LCM 发送线程和第四赛段同时发速度。这里不发额外 STOP，
        # 只是停止旧控制线程；第四赛段会接管后续控制命令。
        try:
            self.Ctrl.quit()
        except Exception as e:
            self.get_logger().warn(f'failed to quit first Ctrl before fourth stage: {e}')

        self.fourth_stage = FourthStageController(self)
    ...
    def control_loop(self):
        ...
        if self.state == 'START_FOURTH_STAGE':
            self.start_fourth_stage_in_same_node()
            return
...
# ============================================================
# 第四赛段代码：由用户上传的第四赛段节点合入，符号做了隔离避免覆盖前三赛段控制类
# ============================================================
...
class FourthYamlPub:
    def __init__(self, parent_node: Node):
        # 使用当前整合节点创建 publisher，不再新建 cyberdogmsg_node。
        self.node = parent_node
        self.para_pub = self.node.create_publisher(YamlParam, "yaml_parameter", 10)
        self.force_pub = self.node.create_publisher(ApplyForce, "apply_force", 10)
...
class FourthRobot_Ctrl(object):
    def __init__(self):
        self.rec_thread = Thread(target=self.rec_responce)
        self.send_thread = Thread(target=self.send_publish)
        self.lc_r = lcm.LCM("udpm://239.255.76.67:7670?ttl=255")
        self.lc_s = lcm.LCM("udpm://239.255.76.67:7671?ttl=255")
        self.cmd_msg = fourth_robot_control_cmd_lcmt()
        self.rec_msg = fourth_robot_control_response_lcmt()
        self.send_lock = Lock()
        self.delay_cnt = 0
        self.mode_ok = 0
        self.gait_ok = 0
    ...
    def msg_handler(self, channel, data):
        self.rec_msg = fourth_robot_control_response_lcmt().decode(data)
...
class FourthStageController:
    def __getattr__(self, name):
        # 让第四赛段原始代码里的 get_logger/create_subscription/create_timer 等
        # 调用自动委托给父 ROS2 节点。这样不会创建第二个 ROS2 节点。
        return getattr(self.node, name)

    def _p4_param_name(self, name: str) -> str:
        # 第四赛段参数全部加 p4_ 前缀，避免和前面一二三赛段的
        # initial_state/control_hz/show_debug_vis/rgb_topic 等参数重名。
        return name if name.startswith('p4_') else f'p4_{name}'

    def declare_parameter(self, name: str, value):
        mapped = self._p4_param_name(name)
        try:
            return self.node.declare_parameter(mapped, value)
        except Exception:
            return self.node.get_parameter(mapped)

    def get_parameter(self, name: str):
        return self.node.get_parameter(self._p4_param_name(name))
    ...
    def __init__(self, parent_node: Node):
        ...
        self.bridge = CvBridge()
        # 复用总节点启动时创建的 yaml publisher；如果没有则兜底创建。
        self.yaml_node = getattr(self.node, 'fourth_yaml_node', None)
        if self.yaml_node is None:
            self.yaml_node = FourthYamlPub(self.node)
            self.node.fourth_yaml_node = self.yaml_node

        # TF：用于后续转向角度和前进距离判断。TransformListener 绑定到父节点。
        self.tf_buffer = Buffer()
        ...
        self.Ctrl = FourthRobot_Ctrl()
        self.Ctrl.run()

        self.msg = fourth_robot_control_cmd_lcmt()
        if not hasattr(self.msg, 'life_count'):
            self.msg.life_count = 0
        ...
        # 第四赛段真正开始时才切换到 low 机身高度，后续第四赛段全程保持 low。
        self.set_body_low()
        # 全流程刚启动时的 normal 高度已在 CombinedStage1Stage2Node.__init__ 中发送。
        # self.send_left_jump_action_once()
        # self.send_left_jump_action_once()
    ...
    def set_body_low(self):
        """
        进入第四赛段时调用一次，把机身高度设置为 low。
        后续第四赛段状态不再重复调用，避免每次切状态都 stop。
        """
...
def main(args=None):
    ...
    try:
        # 一个 ROS2 节点完整跑第一、第二、第三、第四赛段。
        rclpy.spin(node)
    except KeyboardInterrupt:
        pass
    finally:
        try:
            if getattr(node, 'fourth_stage', None) is not None:
                node.get_logger().info('Shutting down fourth stage, sending stop command...')
                node.fourth_stage.send_stop_command()
                try:
                    node.fourth_stage.Ctrl.quit()
                except Exception:
                    pass
            else:
                node.get_logger().info('Shutting down first stages, sending stop command...')
