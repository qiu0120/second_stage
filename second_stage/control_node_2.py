...
class MultiStageOrangeYellowTaskNode(Node):
    def __init__(self):
        ...
        # 默认使用 Gazebo /clock 仿真时间。
        # 如果 launch/yaml 已经声明过 use_sim_time，这里会跳过重复声明；
        # 如果外部没有设置，这里会主动设为 True。
        try:
        ...
        self.declare_parameter('initial_state', 'STAGE3_ROTATE_LEFT_30')
    ...
    def set_state(self, new_state: str):
        if new_state != self.state:
            ...
            if new_state == 'BALL_LATERAL_ALIGN':
                ...
                # 锁定撞后横移方向：在“开始对齐球”的这一刻记录球在图像左/右。
                # 后面对齐过程中球可能因为机器狗横移跑到画面另一侧，
                # 但撞后回中线方向仍按最开始对齐时的 side 来决定。
                target = self.latest_ball_result.get('best_target_ball')
                if target is not None:
                    ...
                    self.get_logger().warn(
                        'BALL_LATERAL_ALIGN entered but no target to lock last_hit_side'
                    )
    ...
    def handle_ball_subchain(self, x: float, y: float, yaw: float) -> bool:
        ...
        if self.state == 'BALL_HIT_CONFIRM_FORWARD':
            ...
            if self.hit_start_time_sec is None:
                ...
                # last_hit_side 不在这里重新记录。
                # 它已经在进入 BALL_LATERAL_ALIGN 的第一刻锁定，
                # 避免对齐过程中目标球从画面左侧跑到右侧后覆盖撞后横移方向。
                if target is not None:
