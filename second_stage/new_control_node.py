...
class MultiStageOrangeYellowTaskNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('blue_min_contour_area', 60.0)
        ...
        # 巡航 / 两排中线巡航
        # =========================
        self.declare_parameter('cruise_forward_speed', 0.10)
        self.declare_parameter('turn_trigger_distance_m', 0.50)

        self.declare_parameter('center_cruise_vy_gain', 0.10)
        self.declare_parameter('center_cruise_vy_max', 0.10)
        self.declare_parameter('center_ok_px', 18.0)
        self.declare_parameter('center_confirm_count', 3)

        # =========================
        # 斜向拦截 / 撞击
        # =========================
        self.declare_parameter('intercept_vy_gain', 0.3)
        self.declare_parameter('intercept_vy_max', 0.3)

        self.declare_parameter('intercept_big_error_px', 120.0)
        self.declare_parameter('intercept_hit_prepare_depth_m', 0.55)
        self.declare_parameter('intercept_hit_prepare_px', 55.0)

        self.declare_parameter('hit_confirm_depth_m', 0.36)
        self.declare_parameter('hit_confirm_px', 35.0)
        self.declare_parameter('hit_confirm_count', 2)

        self.declare_parameter('hit_forward_speed', 0.10)
        self.declare_parameter('hit_forward_duration_sec', 0.55)

        # =========================
        # 撞后回两排中间
        # =========================
        self.declare_parameter('return_forward_speed', 0.06)
        self.declare_parameter('return_center_vy_gain', 0.12)
        self.declare_parameter('return_center_vy_max', 0.10)
        self.declare_parameter('return_center_ok_px', 16.0)
        self.declare_parameter('return_center_confirm_count', 3)
        ...
        self.center_confirm_count = int(self.get_parameter('center_confirm_count').value)

        self.intercept_vy_gain = float(self.get_parameter('intercept_vy_gain').value)
        self.intercept_vy_max = float(self.get_parameter('intercept_vy_max').value)
        self.intercept_big_error_px = float(self.get_parameter('intercept_big_error_px').value)
        self.intercept_hit_prepare_depth_m = float(self.get_parameter('intercept_hit_prepare_depth_m').value)
        self.intercept_hit_prepare_px = float(self.get_parameter('intercept_hit_prepare_px').value)

        self.hit_confirm_depth_m = float(self.get_parameter('hit_confirm_depth_m').value)
        self.hit_confirm_px = float(self.get_parameter('hit_confirm_px').value)
        ...
        self.return_forward_speed = float(self.get_parameter('return_forward_speed').value)
        self.return_center_vy_gain = float(self.get_parameter('return_center_vy_gain').value)
        self.return_center_vy_max = float(self.get_parameter('return_center_vy_max').value)
        self.return_center_ok_px = float(self.get_parameter('return_center_ok_px').value)
        self.return_center_confirm_count = int(self.get_parameter('return_center_confirm_count').value)
        ...
        # 新球子链变量
        self.hit_confirm_counter = 0
        self.return_center_counter = 0
        self.hit_forward_deadline_sec: Optional[float] = None
    ...
    def send_move_right_command(self, speed: float):
        ...
        self.msg.rpy_des = [0.0, 0.0, 0.0]
        self.Ctrl.Send_cmd(self.msg)
        self.get_logger().info(f'[CMD] MOVE RIGHT speed={speed:.3f}', throttle_duration_sec=1.0)
    ...
    def detect_ball_scene(self, frame: np.ndarray) -> Dict:
        ...
        aligned = abs(best_target_ball['error_x']) <= self.hit_confirm_px
    ...
    def set_state(self, new_state: str):
        if new_state != self.state:
            ...
            if new_state == 'BALL_INTERCEPT':
                self.hit_confirm_counter = 0
                self.return_center_counter = 0
                self.hit_forward_deadline_sec = None

            if new_state == 'BALL_HIT_CONFIRM_FORWARD':
                self.hit_forward_deadline_sec = None

            if new_state == 'BALL_RETURN_TO_CENTER':
                self.return_center_counter = 0
    ...
    # 斜向拦截 + 撞击 + 回中线
    # ============================================================
    def compute_intercept_vx(self, depth: float, error_px: float) -> float:
        abs_err = abs(error_px)
        err_norm = clamp(abs_err / max(self.rgb_w * 0.5, 1.0), 0.0, 1.0)
        ...
        # 球偏得越大，vx 越小，防止球离开画面
        vx = vx_base * (1.0 - 0.7 * err_norm)
        return clamp(vx, 0.03, vx_base)

    def compute_intercept_vy(self, error_px: float, depth: float) -> float:
        err_norm = error_px / max(self.rgb_w * 0.5, 1.0)
        vy = clamp(
            -self.intercept_vy_gain * err_norm,
            -self.intercept_vy_max,
            self.intercept_vy_max
        )
    ...
    def handle_ball_subchain(self, x: float, y: float, yaw: float) -> bool:
        ...
        # 1) 斜向拦截
        if self.state == 'BALL_INTERCEPT':
            target = ball['best_target_ball']
            if target is None:
                self.send_stop_command()
                self.set_state('BALL_RETURN_TO_CENTER')
                return True
            ...
            # 接近到准备撞击窗口后，转入短确认
            if depth <= self.intercept_hit_prepare_depth_m and abs(error_px) <= self.intercept_hit_prepare_px:
                self.hit_confirm_counter += 1
                self.get_logger().info(
                    f'BALL_INTERCEPT prepare window ok: '
                    f'depth={depth:.3f}/{self.intercept_hit_prepare_depth_m:.3f}, '
                    f'error={error_px:.1f}/{self.intercept_hit_prepare_px:.1f}, '
                    f'counter={self.hit_confirm_counter}/{self.hit_confirm_count}',
                    throttle_duration_sec=0.2
                )
                if self.hit_confirm_counter >= self.hit_confirm_count:
            ...
            # 球偏差过大时减小前进速度
            vx = self.compute_intercept_vx(depth, error_px)
            vy = self.compute_intercept_vy(error_px, depth)

            self.get_logger().info(
                f'BALL_INTERCEPT target=({target["color"]}, side={target["side"]}) '
                f'depth={depth:.3f} error_x={error_px} radius={radius:.1f} '
        ...
        if self.state == 'BALL_HIT_CONFIRM_FORWARD':
            if self.hit_forward_deadline_sec is None:
                self.hit_forward_deadline_sec = self.now_sec() + self.hit_forward_duration_sec
                self.get_logger().info(
                    f'BALL_HIT_CONFIRM_FORWARD start, deadline={self.hit_forward_deadline_sec:.2f}'
                )

            target = ball['best_target_ball']
            tiny_vy = 0.0
            if target is not None and target['depth_m'] > 0.22:
                err_norm = target['error_x'] / max(self.rgb_w * 0.5, 1.0)
                tiny_vy = clamp(-0.04 * err_norm, -0.03, 0.03)

            self.send_velocity_command(self.hit_forward_speed, tiny_vy, 0.0)

            if self.now_sec() >= self.hit_forward_deadline_sec:
                self.set_state('BALL_RETURN_TO_CENTER')
            return True

        # 3) 撞后回当前两排中线
        if self.state == 'BALL_RETURN_TO_CENTER':
            if ball['has_center_reference'] and ball['center_error_px'] is not None:
                err_px = ball['center_error_px']
                err_norm = err_px / max(self.rgb_w * 0.5, 1.0)
                vy = clamp(
                    -self.return_center_vy_gain * err_norm,
                    -self.return_center_vy_max,
                    self.return_center_vy_max
                )

                self.get_logger().info(
                    f'BALL_RETURN_TO_CENTER center_error_px={err_px:.1f}, '
                    f'vy={vy:.3f}, counter={self.return_center_counter}/{self.return_center_confirm_count}',
                    throttle_duration_sec=0.3
                )

                if abs(err_px) <= self.return_center_ok_px:
                    self.return_center_counter += 1
                    self.send_velocity_command(self.return_forward_speed, vy, 0.0)

                    if self.return_center_counter >= self.return_center_confirm_count:
            ...
            self.return_center_counter = 0
            self.get_logger().info(
                'BALL_RETURN_TO_CENTER no center reference, slow forward fallback.',
                throttle_duration_sec=0.5
            )
            self.send_velocity_command(0.04, 0.0, 0.0)
            return True
